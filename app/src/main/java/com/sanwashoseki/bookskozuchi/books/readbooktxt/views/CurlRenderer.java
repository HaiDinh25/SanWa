...
public class CurlRenderer implements GLSurfaceView.Renderer {
     ...
     * Basic constructor.
     */
    public CurlRenderer(Observer observer, boolean isVertical) {
        mObserver = observer;
    ...
    public void onSurfaceChanged(GL10 gl, int width, int height) {
        ...
        if (USE_PERSPECTIVE_PROJECTION) {
            ...
            if (isVertical) {
                GLU.gluOrtho2D(gl, mViewRect.right, mViewRect.left, mViewRect.bottom, mViewRect.top);
            } else {
    ...
    public void translate(PointF pt) {
        //sua
        if (isVertical) {
            pt.x = mViewRect.right - (mViewRect.width() * pt.x / mViewportWidth);
        } else {
