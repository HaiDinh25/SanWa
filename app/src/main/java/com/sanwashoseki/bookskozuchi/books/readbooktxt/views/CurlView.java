...
public class CurlView extends GLSurfaceView implements View.OnTouchListener, CurlRenderer.Observer {
    ...
    private static final int MAX_CURL_SPLITS = 16;
    ...
    private final long mAnimationDurationTime = 300;
    private final PointF mAnimationSource = new PointF();
    private long mAnimationStartTime;
    private final PointF mAnimationTarget = new PointF();
    private int mAnimationTargetEvent;

    private final PointF mCurlDir = new PointF();

    private final PointF mCurlPos = new PointF();
    ...
    private final PointF mDragStartPos = new PointF();
    ...
    private final PointerPosition mPointerPos = new PointerPosition();

    private CurlRenderer mRenderer;
    private boolean mRenderLeftPage = true;
    private SizeChangedObserver mSizeChangedObserver;
    private PageChangedObserver mPageChangedObserver;
     ...
     * Default constructor.
     */
    public CurlView(Context ctx, boolean isVertical) {
        super(ctx);
    ...
    private void init(Context ctx) {
        mRenderer = new CurlRenderer(this, isVertical);
        ...
        mPageLeft = new CurlMesh(MAX_CURL_SPLITS);
        mPageRight = new CurlMesh(MAX_CURL_SPLITS);
        mPageCurl = new CurlMesh(MAX_CURL_SPLITS);
    ...
    public void setVertical(boolean isVertical) {
        this.isVertical = isVertical;
        mRenderer.setVertical(isVertical);
        onPause();
        onResume();
    }
    ...
    public void onDrawFrame() {
        ...
        if (!mAnimate) {
            if (mCurrentIndex != mLastIndex && mCurrentIndex >= 0) {
                ...
                // Log.e(TAG, "onDrawFrame mAnimate = false " + mLastIndex);
                if (mPageChangedObserver != null) {
                    mPageChangedObserver.onPageChanged(mCurrentIndex, mPageProvider.getPageCount());
                }
            }
            return;
        }
        // Log.e(TAG, "onDrawFrame mAnimate = true");
        long currentTime = System.currentTimeMillis();
        // If animation is done.
        if (currentTime >= mAnimationStartTime + mAnimationDurationTime) {
            // Log.e(TAG, "onDrawFrame currentTime more " + currentTime);
            if (mAnimationTargetEvent == SET_CURL_TO_RIGHT) {
                // Switch curled page to right.
                CurlMesh right = mPageCurl;
                CurlMesh curl = mPageRight;
                right.setRect(mRenderer.getPageRect(CurlRenderer.PAGE_RIGHT), isVertical);
                ...
                left.setRect(mRenderer.getPageRect(CurlRenderer.PAGE_LEFT), isVertical);
            ...
            // Log.e(TAG, "onDrawFrame currentTime less " + currentTime);
        }
    }
    ...
    public boolean onTouch(View view, MotionEvent me) {
        ...
        switch (me.getAction()) {
            ...
            case MotionEvent.ACTION_CANCEL:
                // Log.e(TAG, "onTouch MotionEvent.ACTION_CANCEL");
            case MotionEvent.ACTION_UP: {
                // Log.e(TAG, "onTouch MotionEvent.ACTION_UP " + mCurrentIndex);
    ...
    /**
     * Sets PageChangedObserver for this View. Call back method is called from
     * this View's onPageChanged method.
     */
    public void setPageChangedObserver(PageChangedObserver observer) {
        mPageChangedObserver = observer;
    }
        ...
        switch (page) {
            ...
            case CURL_RIGHT: {
                ...
                if (mCurrentIndex > 0) {
                    mPageLeft.setFlipTexture(true);
                    mPageLeft
                            .setRect(mRenderer.getPageRect(CurlRenderer.PAGE_LEFT), isVertical);
                    mPageLeft.reset();
                    if (mRenderLeftPage) {
                ...
                if (mCurrentIndex < mPageProvider.getPageCount() - 1) {
                    ...
                    mPageRight.setRect(mRenderer
                            .getPageRect(CurlRenderer.PAGE_RIGHT), isVertical);
                    mPageRight.setFlipTexture(false);
                    mPageRight.reset();
                ...
                mPageCurl.setRect(mRenderer.getPageRect(CurlRenderer.PAGE_RIGHT), isVertical);
            ...
            case CURL_LEFT: {
                ...
                if (mCurrentIndex > 1) {
                    ...
                    mPageLeft.setFlipTexture(true);
                    mPageLeft.setRect(mRenderer.getPageRect(CurlRenderer.PAGE_LEFT), isVertical);
                    mPageLeft.reset();
                    if (mRenderLeftPage) {
                        mRenderer.addCurlMesh(mPageLeft);
                    }
    ...
    public void update(int index){
        // Log.e(TAG, "update " + index);
        updatePage(mPageCurl.getTexturePage(), index);
    }

    private void updatePage(CurlPage page, int index) {
        // Log.e(TAG, "updatePage " + index);
        page.reset();
    ...
    private void updatePages() {
        ...
        if (rightIdx >= 0 && rightIdx < mPageProvider.getPageCount()) {
            ...
            mPageRight.setFlipTexture(false);
            mPageRight.setRect(mRenderer.getPageRect(CurlRenderer.PAGE_RIGHT), isVertical);
            mPageRight.reset();
            mRenderer.addCurlMesh(mPageRight);
        }
        ...
        if (curlIdx >= 0 && curlIdx < mPageProvider.getPageCount()) {
            ...
            if (mCurlState == CURL_RIGHT) {
                ...
                mPageCurl.setFlipTexture(false);
                mPageCurl.setRect(mRenderer.getPageRect(CurlRenderer.PAGE_LEFT), isVertical);
            }
    ...
    private static class PointerPosition {
