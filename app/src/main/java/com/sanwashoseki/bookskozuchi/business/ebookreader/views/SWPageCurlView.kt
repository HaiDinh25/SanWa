...
class SWPageCurlView : View {
    ...
    private inner class Vector2D(var x: Float, var y: Float) {
        ...
        fun normalize(): Vector2D {
            val magnitude =
                sqrt(dotProduct(this).toDouble()).toFloat()
    ...
    constructor(context: Context, attrs: AttributeSet?) : super(context, attrs) {
        ...
        Log.d(TAG, "mCurlSpeed: $mCurlSpeed")
        Log.d(TAG, "mUpdateRate: $mUpdateRate")
        Log.d(TAG, "mInitialEdgeOffset: $mInitialEdgeOffset")
        Log.d(TAG, "mCurlMode: $mCurlMode")
    ...
    fun SetCurlMode(curlMode: Int) {
        require(
            !(curlMode != CURLMODE_SIMPLE && curlMode != CURLMODE_DYNAMIC)) { "Invalid curlMode" }
        mCurlMode = curlMode
    }
        ...
        if (point!!.distance(mOrigin) > mFlipRadius) {
            if (bMaintainMoveDir) {
                ...
                if (point.x > mOrigin!!.x + mFlipRadius)
                    point.x = mOrigin!!.x + mFlipRadius else if (point.x < mOrigin!!.x - mFlipRadius) point.x = mOrigin!!.x - mFlipRadius
                point.y = (sin(acos(Math.abs(point.x - mOrigin!!.x) / mFlipRadius.toDouble())) * mFlipRadius).toFloat()
            }
        }
        return point
    }
     ...
     * @param canvas
     */
    private fun onFirstDrawEvent(canvas: Canvas?) {
