...
import com.sanwashoseki.bookskozuchi.business.ebookstore.services.SWBookStoreHighLightInterface
import com.sanwashoseki.bookskozuchi.business.ebookstore.services.SWBookStoreHighLightPresenter
...
class SWHighlightBookFragment : SWBaseFragment(), SWHighlightBooksAdapter.OnItemClickListener,
    SWMainActivity.OnHeaderClickedListener, SWBookStoreHighLightInterface, SWFilterBookInterface {

    private var binding: FragmentHighlightBookBinding? = null
    private var adapter: SWHighlightBooksAdapter? = null
    private var preGetBook: SWBookStoreHighLightPresenter? = null
    ...
    companion object {
        @JvmStatic
        fun newInstance(filter: Boolean?, request: SWFilterBookRequest?) = SWHighlightBookFragment()
            .apply {
    ...
    private fun initUI() {
        ...
        (activity as SWMainActivity).setHeaderInChildrenScreen(getString(R.string.book_store_highlight), isSearch = false, isFilter = true)
        (activity as SWMainActivity).onCallBackSearchListener(this)

        val ids = ArrayList<Int>()
        if (isFilter == false) {
            preGetBook?.getBookStoreHighLight(context, SWConstants.MAX_VALUE, true)
        } else {
            ...
            preFilterBook?.filterHighLightBook(context, true, filterModel?.categoryIds, filterModel?.bookType, filterModel?.priceMin, filterModel?.priceMax, ids)
        }
        binding?.let { it ->
        ...
        binding?.rcListBook?.addOnScrollListener(object : RecyclerView.OnScrollListener() {
            override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
                ...
                if (!isLoading && currentItem == (adapter?.itemCount?.minus(8))) {
                    isLoading = true
                    preGetBook?.loadMoreBooks(context, SWConstants.MAX_VALUE)
                }
            }
        })
    }
    ...
    override fun onAttach(context: Context) {
        ...
        preGetBook = SWBookStoreHighLightPresenter()
...
//        replaceFragment(SWBookStoreFragment.newInstance(), R.id.container, true, null)
    ...
    override fun onFilterClicked() {
        replaceFragment(SWFilterBookFragment.newInstance(false,  Const.FILTER_HIGH_LIGHT, filterModel), R.id.container, true, null)
    }
