...
class SWAboutUsPresenter : MVP<SWAboutUsInterface> {

    var view: SWAboutUsInterface? = null
    var disposable = CompositeDisposable()

    fun getAboutUs(context: Context?) {
        view?.showIndicator()
        if (NetworkUtil.isNetworkConnected(context)) {
            disposable.add(RetrofitClient.client?.getAboutUs()!!
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe (
                    { result: SWAboutUsResponse? ->
                        if (result?.success == true) {
                            view?.hideIndicator()
                            view?.getAboutUsSuccess(result)
                        } else {
                            view?.hideIndicator()
    ...
    override fun attachView(view: SWAboutUsInterface) {
        this.view = view
    }
